...
var BASE_PACKAGE = {
  description: 'A full Quantisnet node build with Quantisnetcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@quantisnetevo/quantisnetcore-lib': '^' + quantisnetcore.version,
    '@quantisnetevo/quantisnetcore-node': version
  }
};

/**
 * Will create a directory and quantisnet.conf file for Quantisnet.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createQuantisnetDirectory(datadir, done) {
...
/**
 * Will create a base Quantisnetcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The quantisnet database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/quantisnetcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Quantisnetcore Node directory, configuration file,
 * quantisnet configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the quantisnetcore node configuration directory
 * @param {String} options.datadir - The path to the quantisnet datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the quantisnetcore-node directory and configuration
    ...
    function(next) {
      // Setup the Quantisnet directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createQuantisnetDirectory(absDataDir, next);
      } else {
        next();
      }
    },
