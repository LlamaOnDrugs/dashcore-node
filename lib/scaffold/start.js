...
var QuantisnetcoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for quantisnetd is necessary for this upgrade with the "reindex=1" quantisnet.conf option. \n' +
                 'There are changes necessary in both quantisnet.conf and quantisnetcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/quantisnetevo/quantisnetcore-node/blob/quantisnetd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          quantisnetd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/quantisnetd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed quantisnetcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './quantisnetcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in quantisnetcore-node services directory
    ...
    if (servicePackage.quantisnetcoreNode) {
      serviceModule = service.name + '/' + servicePackage.quantisnetcoreNode;
    }
 ...
 * @param {Node} node - The Quantisnetcore Node instance
 */
