...
/**
 * Will return the path and default quantisnetcore-node configuration. It will search for the
 * configuration file in the "~/.quantisnetcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.quantisnetcore');
  var defaultConfigFile = path.resolve(defaultPath, './quantisnetcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['quantisnetd', 'web'];
