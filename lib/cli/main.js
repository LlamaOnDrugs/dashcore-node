...
var quantisnetcorenode = require('..');
var utils = require('../utils');

function main(servicesPath, additionalServices) {
  /* jshint maxstatements: 100 */

  var version = quantisnetcorenode.version;
  var create = quantisnetcorenode.scaffold.create;
  var add = quantisnetcorenode.scaffold.add;
  var start = quantisnetcorenode.scaffold.start;
  var remove = quantisnetcorenode.scaffold.remove;
  var callMethod = quantisnetcorenode.scaffold.callMethod;
  var findConfig = quantisnetcorenode.scaffold.findConfig;
  var defaultConfig = quantisnetcorenode.scaffold.defaultConfig;
  ...
  program
    ...
    .option('-d, --datadir <dir>', 'Specify the Quantisnet database directory')
  ...
  program
    ...
    .option('-d, --daemon', 'Make quantisnetcore-node a daemon (running in the background)')
    .action(function(cmd){
      if (cmd.config) {
  ...
  program
    ...
    .action(function(services){
      ...
      if (!configInfo) {
        throw new Error('Could not find configuration, see `quantisnetcore-node create --help`');
      }
      var opts = {
      ...
      console.log();
      console.log('    $ quantisnetcore-node add wallet-service');
      console.log('    $ quantisnetcore-node add insight-api');
      console.log();
    });
  ...
  program
    ...
    .action(function(services){
      ...
      console.log('  Examples:');
      console.log();
      console.log('    $ quantisnetcore-node remove wallet-service');
      console.log('    $ quantisnetcore-node remove insight-api');
      console.log();
    });
