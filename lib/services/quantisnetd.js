...
/**
 * Provides a friendly event driven API to quantisnetd in Node.js. Manages starting and
 * stopping quantisnetd as a child process for application support, as well as connecting
 * to multiple quantisnetd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Quantisnet(options) {
  if (!(this instanceof Quantisnet)) {
    return new Quantisnet(options);
  }
...
util.inherits(Quantisnet, Service);

Quantisnet.dependencies = [];

Quantisnet.DEFAULT_MAX_TXIDS = 1000;
Quantisnet.DEFAULT_MAX_HISTORY = 50;
Quantisnet.DEFAULT_MAX_UTXO = 1000;
Quantisnet.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Quantisnet.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Quantisnet.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Quantisnet.DEFAULT_SPAWN_RESTART_TIME = 5000;
Quantisnet.DEFAULT_SPAWN_STOP_TIME = 10000;
Quantisnet.DEFAULT_TRY_ALL_INTERVAL = 1000;
Quantisnet.DEFAULT_REINDEX_INTERVAL = 10000;
Quantisnet.DEFAULT_START_RETRY_INTERVAL = 5000;
Quantisnet.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Quantisnet.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Quantisnet.DEFAULT_INSTANTSEND_FEE = 10000;
Quantisnet.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'quantisnetcore'
};

Quantisnet.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Quantisnet.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Quantisnet.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Quantisnet.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Quantisnet.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Quantisnet.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Quantisnet.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Quantisnet.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Quantisnet.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Quantisnet.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Quantisnet.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Quantisnet.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Quantisnet.prototype._initCaches = function() {
...
Quantisnet.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Quantisnet.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Quantisnet.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'quantisnetd/transactionlock',
      scope: this,
...
Quantisnet.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'quantisnetd/' + name, 'total:', this.subscriptions[name].length);
};

Quantisnet.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'quantisnetd/' + name, 'total:', this.subscriptions[name].length);
};

Quantisnet.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (quantisnetcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'quantisnetd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Quantisnet.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'quantisnetd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Quantisnet.prototype.unsubscribeAddressAll = function(emitter) {
...
Quantisnet.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Quantisnet.DEFAULT_CONFIG_SETTINGS;
...
Quantisnet.prototype._parseQuantisnetConf = function(configPath) {
  var options = {};
...
Quantisnet.prototype._expandRelativeDatadir = function() {
...
Quantisnet.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in quantisnetd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in quantisnetd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in quantisnetd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './quantisnet.conf');

  log.info('Using Quantisnet config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseQuantisnetConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseQuantisnetConf(networkConfigPath));
  }
...
Quantisnet.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of quantisnetcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of quantisnetcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of quantisnetcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to quantisnetd from quantisnetcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from quantisnetd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
      spawnConfig.zmqpubrawtxlock,
      '"zmqpubrawtxlock" option is required to get transaction locks from quantisnetd. ' +
      'Please add "zmqpubrawtxlock=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from quantisnetd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in quantisnet.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that quantisnetd is undergoing a reindex. ' +
             ...
             'of quantisnetcore-node services will start.');
    node._reindex = true;
  }
};

Quantisnet.prototype._resetCaches = function() {
 ...
 * @private
 */
Quantisnet.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Quantisnet.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Quantisnet.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Quantisnet Daemon Ready');
          callback();
        });
      });
...
Quantisnet.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === quantisnetcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Quantisnet.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === quantisnetcore.Networks.testnet) {
    networkPath = 'testnet3/quantisnet.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/quantisnet.conf';
    }
  }
  return networkPath;
};

Quantisnet.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === quantisnetcore.Networks.testnet) {
...
Quantisnet.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('quantisnetd/hashblock', message.toString('hex'));
    }
  }

};

Quantisnet.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Quantisnet.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Quantisnet Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Quantisnet.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Quantisnet.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('quantisnetd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Quantisnet.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = quantisnetcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('quantisnetd/rawtransaction', message.toString('hex'));
    }

    var tx = quantisnetcore.Transaction();
    tx.fromString(message);
    var txid = quantisnetcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Quantisnet.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = quantisnetcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('quantisnetd/transactionlock', message.toString('hex'));
    }

  }
};

Quantisnet.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Quantisnet.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Quantisnet.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Quantisnet.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Quantisnet.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Quantisnet Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Quantisnet.DEFAULT_REINDEX_INTERVAL);

};

Quantisnet.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Quantisnet.prototype._stopSpawnedQuantisnet = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/quantisnetd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned quantisnet process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean quantisnet process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Quantisnet.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedQuantisnet(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting quantisnet process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Quantisnet process unexpectedly exited with code:', code);
        log.warn('Restarting quantisnet child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Quantisnet process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new QuantisnetdRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn quantisnetd.'));
      }
...
Quantisnet.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to quantisnetd.'));
    }
 ...
 */
Quantisnet.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Quantisnet configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.syncPercentage = function(callback) {
  var self = this;
...
Quantisnet.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, quantisnetcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = quantisnetcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Quantisnet.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Quantisnet.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Quantisnet.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in quantisnetcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Quantisnet.prototype._getConfirmationsDetail = function(transaction) {
...
Quantisnet.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Quantisnet.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Quantisnet.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A quantisnet transaction id
 * @param {Function} callback
 */
Quantisnet.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Quantisnet.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof quantisnetcore.Address) {
...
Quantisnet.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Quantisnet.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Quantisnetcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Quantisnet.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = quantisnetcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Quantisnet.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Quantisnet.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Quantisnetcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Quantisnet.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Quantisnet.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Quantisnet.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Quantisnet.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Quantisnet.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Quantisnet.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Quantisnet.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Quantisnet.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Quantisnet.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Quantisnet.prototype.govCount = function (callback) {
  var self = this;
...
Quantisnet.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Quantisnet.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Quantisnet.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Quantisnet.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.getInfo = function(callback) {
  var self = this;
...
Quantisnet.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Quantisnet.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('quantisnetd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('quantisnetd process did not exit'));
      }
